...
public class Slot {
    ...
    public List<Float> getChancesMin(List<Float> droppers,List<String> combinations){
        List<DropperChance> chances = new ArrayList<>();
        for(int i=0;i<=droppers.size();i++){
            chances.add(new DropperChance(0));
        }
        ...
        List<Float> minChances = new ArrayList<>();
        for(int i=0;i<chances.size();i++){
            float chance = 0;
            for(int j=i;j<chances.size();j++){
                chance+=chances.get(j).chance;
            }
            minChances.add(chance);
        }
        return minChances;
    }
